...
export const TipoProductoCreate = () => {
    ...
    const handleChange = (event) => {
        ...
        setTipoProducto((tipoProductoCurrent) => ({ ...tipoProductoCurrent, [name]: value }));
    };
    ...
    const handleSubmit = (event) => {
        ...
        if (event.target.checkValidity() === true) {
            ...
            clientHttp.post(`/TipoProducto`, tipoProducto)
                .then((response) => {
            ...
            <div className="col-6">
                ...
                <div className="input-group has-validation">
                    <input type="text" className="form-control" id="id"
                        value={id} onChange={e => handleChange(e)} required maxLength="30" />
...
export const TipoProductoEdit = () => {
    ...
    useEffect(() => {
        clientHttp.get(`/TipoProducto/${tipoProductoId}`)
            .then((response) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (event.target.checkValidity() === true) {
            //console.log("Enviar...");
            //console.log(client);
            clientHttp.put(`/TipoProducto/?id=${tipoProductoId}`, tipoProducto)
                .then((response) => {
                    navegacion(`/admin/type-products`)
                });
        }
    ...
    return loading ? <div>Loading data...</div> :
        <form className="row" onSubmit={(e) => handleSubmit(e)}>
            ...
            <div className="col-12 mt-3">
                <button className="btn btn-secondary" type="button" onClick={(e) => navegacion(`/admin/type-products`)}>Cancelar</button>
                <button className="btn btn-primary ms-3" type="submit">Guardar</button>
...
export const TipoProductoEliminar = () => {
    ...
    const handleSubmit = (event) => {
        ...
        if (event.target.checkValidity() === true) {
            //console.log("Enviar...");
            //console.log(client);
            clientHttp.delete(`/TipoProducto/?marcaId=${tipoProductoId}`, tipoProducto)
                .then((response) => {
                    navegacion(`/admin/type-products`)
                });
        }
